...
use mm_ptr::x_deps::{abs_mm, abs_sync, atomex, atomic_sync};
    ...
    O: TrCmpxchOrderings,
{
    ...
    pub fn try_split<S>(this: S) -> Result<Split<S, T, O>, S>
    where
        S: TrShared<Target = Self> + Clone + Send + Sync,
    {
        let x = TrShared::strong_count(&this) > 1 ||
            TrShared::weak_count(&this) > 0;
        if x {
            Result::Err(this)
        } else {
            let sender = Sender::new(this.clone());
            let recver = Receiver::new(this);
...
impl<'a, C, B, T, O> Future for RecvFuture<'a, C, B, T, O>
where
    B: Deref<Target = Oneshot<T, O>>,
    C: TrCancellationToken,
    O: TrCmpxchOrderings,
...
mod tests_ {
    ...
    async fn tx_rx_smoke() {
        ...
        let Result::Ok((mut tx, rx)) = Oneshot::try_split(oneshot)
        else {
    ...
    async fn peeker_smoke() {
        let Result::Ok((mut tx, rx)) = Oneshot::try_split(Shared::new(
            Oneshot::<usize, StrictOrderings>::new(),
            CoreAlloc::new(),
        )) else {
